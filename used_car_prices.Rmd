...
Numerical distribution measures lead to observations:
...
Graphical representation by histogram.
Obersations:
...
Categorical variables that are highly concentrated on one value:
```{r}
kable(t(as.matrix(with(data, table(fahrzeugart)))))
```

```{r}
kable(t(as.matrix(with(data, table(tueren)))))
```

```{r}
kable(t(as.matrix(with(data, table(sitze)))))
```

```{r}
kable(t(as.matrix(with(data, table(zylinder)))))
```


Categorical variables that are multidimensional:
```{r}
