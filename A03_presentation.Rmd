...
\textcolor{black}{2.} Variables are encoded as integers (hubraum, t?ren, etc.), while they should rather be treated as factors.  
...
plot + scale_fill_manual(values=c("red","blue"),name="Energieeffizient is Missing")+labs(x = "Car Price",y = "Count")+ggtitle("Distribution of 'preis' depending on 'energieeffizienz'")+theme(plot.title = element_text(lineheight=2, face="bold"))
```
...
\begin{center}Summary Statistic\end{center}  
```{r,echo=FALSE}
kable(t(as.matrix(summary(data$preis))),format = "markdown")
```

# 3. Descriptive Analysis
\textcolor{black}{4.} **Histogram of Price: What do we observe?**  
...
plot + scale_fill_manual(values=c("red","blue"),name=c("Young"))+labs(x = "Car Price",y = "Count")+ggtitle("Histogram of Price")+theme(plot.title = element_text(lineheight=2, face="bold"))
```
...
numeric.covariates <- numeric.covariates[-which(numeric.covariates=="young")]
...
```
\textcolor{black}{1.} **A linear model with level effects in age:**  
...
# 4. Multivariate Analysis
\textcolor{black}{2.} **A linear model with level effects in age and dummies:**  
...
# 4. Multivariate Analysis
\textcolor{black}{3.} **A linear model with maximum fit:**   
...
```{r,echo=FALSE}
r.sq <- data.frame(name = c("Fit1","Fit2","Fit3"),
r.squared = c(summary(fit2)$r.squared,
summary(fit3)$r.squared,
summary(fit4)$r.squared))
kable(r.sq,format = "markdown")
```


# 4. Multivariate Analysis
**Predicitve Quality of a Model:**  
...
-Output the densitiy distributions of the Root Mean Squared Error  
...
# calculate RMSE's
RMSE1 <- replicate(100, RMSEPredict(formula2, data, alpha=0.8))
RMSE2 <- replicate(100, RMSEPredict(formula3, data, alpha=0.8))
RMSE3 <- replicate(100, RMSEPredict(formula4, data, alpha=0.8))
# create data frame
RMSE.compare <- data.frame(RMSE1, RMSE2, RMSE3)
RMSE.compare <- melt(RMSE.compare)
plot <- ggplot(RMSE.compare, aes(x=value, colour=variable))
plot <- plot + geom_density()
plot + labs(title="Density-distribution")
...
\textcolor{black}{2.} Regression: Built in function methods(class=""), e.g. methods(class="lm")
